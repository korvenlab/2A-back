-- Resgate promocional: usar sempre a organização canónica em app_users (PK = id).
-- Evita falha "Usuário ou organização inválidos" quando o Bearer resolve org via user_roles
-- mas a linha app_users tem organization_id diferente ou o parâmetro p_org_id não coincide.
...
comment on function public.redeem_billing_promo_link(text, uuid, uuid) is
  'Resgate atómico: valida link; organization_id e update vêm só de app_users (p_org_id ignorado, compat API).';
