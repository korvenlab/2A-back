-- Cadastro via Auth: o trigger em auth.users roda sem JWT de sessão, então auth.uid()
-- fica NULL. Políticas RLS em customers / seller_invitations (e funções como current_user_org)
-- deixam de bater → "Database error saving new user". Definimos o sub/role do JWT no
-- contexto da transação antes de inserts/updates que passam por RLS.
