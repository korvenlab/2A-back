-- Cadastro Auth ainda falhando: set_config sozinho pode não alimentar auth.uid() em todos os ambientes,
-- ou o dono de handle_new_user não é o dono das tabelas → RLS bloqueia INSERT em customers / UPDATE em convites.
-- 1) JWT completo em request.jwt.claims (fallback usado por auth.uid() em algumas versões).
-- 2) Funções internas SECURITY DEFINER com OWNER postgres inserem/atualizam sem depender de políticas TO authenticated.
-- 3) handle_new_user passa a ser de postgres quando a migration roda com privilégio suficiente.

create or replace function public.insert_customer_row_from_signup_invite(
...
begin
  ...
  values (
    ...
    p_email,
    p_assigned_seller_id
  );
end;
$$;

create or replace function public.mark_seller_invitation_accepted_from_signup(p_token text)
returns void
language plpgsql
security definer
set search_path = public
...
revoke all on function public.insert_customer_row_from_signup_invite(
  uuid, uuid, text, text, text, text, uuid
) from public;

revoke all on function public.mark_seller_invitation_accepted_from_signup(text) from public;

grant execute on function public.insert_customer_row_from_signup_invite(
  uuid, uuid, text, text, text, text, uuid
) to postgres;

grant execute on function public.mark_seller_invitation_accepted_from_signup(text) to postgres;
...
begin
  if exists (select 1 from pg_roles where rolname = 'supabase_auth_admin') then
    ...
    grant execute on function public.mark_seller_invitation_accepted_from_signup(text) to supabase_auth_admin;
  end if;
end
$do$;

alter function public.insert_customer_row_from_signup_invite(uuid, uuid, text, text, text, text, uuid) owner to postgres;
alter function public.mark_seller_invitation_accepted_from_signup(text) owner to postgres;
